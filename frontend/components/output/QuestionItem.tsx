...
export const QuestionItem = memo(function QuestionItem({ question }: { question: GeneratedQuestion }) {
  ...
  return (
    <div className="rounded-xl border-b border-dashed border-[var(--border)] pb-5">
      <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:gap-5">
        ...
        <div className="no-print flex shrink-0 items-start gap-2 sm:justify-end">
          ...
          <span className="pill border border-[var(--border)] bg-[var(--surface-subtle)] px-2.5 py-1 text-xs font-semibold text-[var(--text)]">{question.marks} {question.marks === 1 ? 'Mark' : 'Marks'}</span>
        </div>
      </div>
      {question.options.length ? (
        <div className="mt-3 grid gap-2 pl-8 text-[13.5px] leading-6 text-[var(--text-soft)] sm:grid-cols-2">
